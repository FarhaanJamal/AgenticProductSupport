...
import pandas as pd
import os
from product_support.crews.hypothesis_crew.hypothesis_crew import HypothesisCrew
from product_support.crews.main_crew.main_crew import MainCrew
...
def get_machine_details(qr_file_location):
    print("getting machine details")
    qr_image = Image.open(qr_file_location)
...
def get_initial_hypothesis(machine_details):
    ...
    inputs = {"machine_details": machine_details}
    initial_hypothesis = hypothesis_crew.crew().kickoff(inputs)
    cleaned_hypothesis = (
        initial_hypothesis.raw.replace("```python", "").replace("```", "").strip()
    )
    initial_hypothesis = eval(cleaned_hypothesis)
    # print(f"Initial hypothesis: {initial_hypothesis}")

    machine_sentence = f"""
    This is a {machine_details['Type']} washing machine with a load capacity of {machine_details['Load Capacity']}.
    It has an energy rating of {machine_details['Energy Rating']} and operates at {machine_details['RPM']}. 
    Key features include {machine_details['Key Features']}.
    """
    # print([machine_sentence] + initial_hypothesis)
    return [machine_sentence] + initial_hypothesis
...
if __name__ == "__main__":
    machine_details = get_machine_details("path")
    ...
    conversation_history, final_result = call_main_crew(
        user_prompt, machine_details, conversation_history
    )
